...
class MainWindow(QMainWindow):
    ...
    def fetch_url(self):
        ...
        try:
            ...
            # 增量保存回调
            output_dir = self.output_dir
            def save_callback(html_content, page_title):
                safe_title = "".join(c if c.isalnum() or c in ' -_' else '_' for c in page_title)[:50]
                path = Path(output_dir) / f"{safe_title}.{fmt.lower()}"
                if fmt == "HTML":
                    with open(path, 'w', encoding='utf-8') as f:
                        f.write(html_content)
                else:  # MD, PDF, DOCX 都先转MD
                    md = html_to_markdown(html_content)
                    with open(path.with_suffix('.md'), 'w', encoding='utf-8') as f:
                        f.write(md)

            content, title = do_fetch(url, main_only=main_only, use_browser=use_browser, save_callback=save_callback)
            # 最终保存
